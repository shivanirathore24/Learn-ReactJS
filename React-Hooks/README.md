...
1. Hard to reuse stateful logic between components - Hooks provide a way to separate stateful logic from a component, which
...
2. Complex components become difficult to understand - In class-based components, related code often gets mixed with unrelated
...
3. Classes can be confusing at times - Classes in React can be a significant hurdle for learning the framework. Not
...
1. Only call Hooks at the top level: - This rule states that Hooks should only be called at the top level of a function
   ...
   bugs.
2. Only call Hooks from React function components: - This rule states that Hooks can only be used in React function components or
...
1. Syntax: - Class-based components have lifecycle methods, such as componentDidMount
...
2. Updates and Side effects: - In class-based components, state is updated via the setState method. In functional
...
3. Boilerplate: - Class-based components require more boilerplate code to manage state and
...
1. Parameters - Intial State: The useState hook in React takes an initial state as a parameter.
   ...
   the return value as the initial state. - The initial state is only used during the first render and subsequent calls to
   useState with a new initial state will override the previous initial state.

2. Returns
   The useState hook returns an array with exactly two values: - The current state. During the first render, it will match the initialState you have
   passed. - The set function that lets you update the state to a different value and trigger
   a re-render.
...
1. Parameters - Setup: - The useEffect hook in React takes a function as its first argument,
   ...
   values. - Finally, when your component is removed from the DOM, React will execute
   your cleanup function one last time. This ensures that your component's side
   effects are properly added and removed throughout its lifecycle. - Options (dependencies): - The useEffect hook in React takes a dependency
   ...
   the form of [dep1, dep2, dep3]. - React compares each dependency with its previous value using a comparison
...
The added code introduces another useEffect hook that is responsible for displaying the window's width at regular intervals in the console.
Explanation:

1. First useEffect Hook (Title Update):
   - This hook updates the document's title to the current value of name and lastName every time either of them changes.
   - Since there's no dependency array ([]), this useEffect runs every time the component re-renders, similar to how componentDidMount and componentDidUpdate work in class components.
2. Second useEffect Hook (Window Width Logging):
   - This hook is used to log the current window width to the console every 2 seconds.
   - setInterval is used to set up a timer that logs the window width repeatedly.
   - The return statement in the useEffect acts as a cleanup function that clears the interval when the component is unmounted or before the effect is re-executed (which happens on every render since no dependency array is provided).
   - This ensures that you don't accidentally create multiple timers if the component re-renders often.

Key Points:

1. Separation of Concerns: The two useEffect hooks are used to handle different side effects independently. The first one updates the document title, while the second one logs the window width.
2. Cleanup Function: The second useEffect includes a cleanup function (clearInterval(timer)) to prevent memory leaks by clearing the interval when it's no longer needed.
3. Component Lifecycle: In class components, you would typically manage side effects in componentDidMount and componentWillUnmount methods. In functional components, this is handled more flexibly using useEffect.

This pattern keeps the component's logic modular and clean, making it easier to manage and understand.
